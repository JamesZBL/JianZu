...
public class PopupWindowUtils {
	...
	public static PopupWindow getPopupWindowAtLocation(View contentView, int width, int height, View parentView, int gravityType, int xoff, int yoff, int animationStyle) {
		PopupWindow popupWindow = getPopupWindow(contentView, width, height);
	...
	public static PopupWindow getPopupWindowAtLocation(View contentView, View parentView, int gravityType, int xoff, int yoff, int animationStyle, Activity activity) {
		//背景变暗
		UIUtils.setWindowAlpha(activity, AppConst.SCRN_DIM_AMOUNT);
		PopupWindow window = getPopupWindowAtLocation(contentView, ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT, parentView, gravityType, xoff, yoff, animationStyle);
		window.setOnDismissListener(() -> UIUtils.setWindowAlpha(activity, 1.0f));//完全不暗);
		return window;
	}
